...
class CarlaRosBridge(Node):
    ...
    def initialize_bridge(self, carla_world, params):
        ...
        self.spawn_object_service = self.create_service(SpawnObject, "/carla/spawn_object", self.spawn_object)
        self.destroy_object_service = self.create_service(DestroyObject, "/carla/destroy_object", self.destroy_object)
    ...
    def _synchronous_mode_update(self):
        ...
        while not self.shutdown.is_set() and rclpy.ok():
            ...
            self.get_logger().debug("Tick for frame {} returned. Waiting for sensor data...".format(
                frame))
            self._update(frame, world_snapshot.timestamp.elapsed_seconds)
            self.get_logger().debug("Waiting for sensor data finished.")
...
def main(args=None):
    ...
    on_shutdown(carla_bridge.destroy)
