...
class DebugHelper(object):
    ...
    def destroy(self):
        ...
        :return:
        """
        self.node.get_logger().debug("Destroy DebugHelper")
        self.debug = None
    ...
    def on_marker(self, marker_array):
        ...
        for marker in marker_array.markers:
            if marker.header.frame_id != "map":
                self.node.get_logger().warn("Could not draw marker in frame '{}'. Only 'map' supported.".format(
    ...
    def draw_arrow(self, marker, lifetime, color):
        ...
        if marker.points:
            if not len(marker.points) == 2:
                self.node.get_logger().warn("Drawing arrow from points requires two points. Received {}".format(
                    len(marker.points)))
                return
            ...
            self.node.get_logger().info("Draw Arrow from {} to {} (color: {}, lifetime: {}, "
            ...
            self.node.get_logger().warn("Drawing arrow from Position/Orientation not yet supported. "
                              "Please use points.")
    ...
    def draw_points(self, marker, lifetime, color):
        ...
        for point in marker.points:
            ...
            size = marker.scale.x
            self.node.get_logger().info("Draw Point {} (color: {}, lifetime: {}, size: {})".format(
    ...
    def draw_line_strips(self, marker, lifetime, color):
        ...
        if len(marker.points) < 2:
            self.node.get_logger().warn("Drawing line-strip requires at least two points. Received {}".format(
                len(marker.points)))
            return
        last_point = None
        thickness = marker.scale.x
        for point in marker.points:
            if last_point:
                ...
                self.node.get_logger().info("Draw Line from {} to {} (color: {}, lifetime: {}, "
    ...
    def draw_box(self, marker, lifetime, color):
        ...
        self.node.get_logger().info("Draw Box {} (rotation: {}, color: {}, lifetime: {})".format(
