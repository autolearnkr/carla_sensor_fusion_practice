...
class Sensor(Actor):
    ...
    def __init__(self,  # pylint: disable=too-many-arguments
        ...
        try:
            ...
            node.get_logger().debug("Sensor tick time is {}".format(self.sensor_tick_time))
    ...
    def get_ros_transform(self, pose, timestamp):
        if self.synchronous_mode:
            if not self.relative_spawn_pose:
                self.node.get_logger().warn("{}: No relative spawn pose defined".format(self.get_prefix()))
                return
        ...
        transform.header.stamp = rclpy.time.Time(seconds=timestamp).to_msg()
    ...
    def publish_tf(self, pose, timestamp):
        ...
        try:
            ...
            if rclpy.ok():
                self.node.get_logger().warn("Sensor {} failed to send transform.".format(self.uid))
        ...
        while not self.next_data_expected_time or \
            ...
            while True:
                try:
                    ...
                    if carla_sensor_data.frame == frame:
                        self.node.get_logger().debug("{}({}): process {}".format(self.__class__.__name__,
                                                                       self.get_id(), frame))
                        self.publish_tf(trans.carla_transform_to_ros_pose(
                            carla_sensor_data.transform), timestamp)
                        self.sensor_data_updated(carla_sensor_data)
                        ...
                        self.node.get_logger().warn("{}({}): skipping old frame {}, expected {}".format(
                    ...
                    if rclpy.ok():
                        self.node.get_logger().warn("{}({}): Expected Frame {} not received".format(
                            self.__class__.__name__, self.get_id(), frame))
                    return
