...
class ActorFactory(object):
        ...
        with self.lock:
            for actor_id in self.actors:
                try:
                    ...
                    self.node.get_logger().warn("Update actor {}({}) failed: {}".format(
    ...
    def _destroy_object(self, actor_id, delete_actor):
        ...
        if carla_actor and delete_actor:
            carla_actor.destroy()
        self.node.get_logger().info("Removed {}(id={})".format(actor.__class__.__name__, actor.uid))
    ...
    def _create_object(self, uid, type_id, name, attach_to, spawn_pose, carla_actor=None):
        ...
        self.node.get_logger().info("Created {}(id={})".format(actor.__class__.__name__, actor.uid))
