...
"""
LiDAR processor using manual serialization (tobytes() byte stream mapping).
This bypasses the create_cloud helper for low-level serialization training.
"""
...
class Lidar(Sensor):
    ...
    def sensor_data_updated(self, carla_lidar_measurement):
        """
        Transform using manual binary serialization (tobytes() mapping).
        """
        ...
        # [tobytes() 수동 바이너리 직렬화 방식]
        point_cloud_msg = PointCloud2()
        point_cloud_msg.header = header
        point_cloud_msg.height = 1
        point_cloud_msg.width = len(structured_lidar_data)
        point_cloud_msg.fields = fields
        point_cloud_msg.is_bigendian = False
        point_cloud_msg.point_step = 16  # f4*3 (12) + u1*2 (2) + u2 (2) = 16 Bytes
        point_cloud_msg.row_step = point_cloud_msg.point_step * point_cloud_msg.width
        point_cloud_msg.is_dense = True
        
        # NumPy 구조화 배열의 메모리 버퍼를 bytes로 직접 복사하여 주입
        point_cloud_msg.data = structured_lidar_data.tobytes()
