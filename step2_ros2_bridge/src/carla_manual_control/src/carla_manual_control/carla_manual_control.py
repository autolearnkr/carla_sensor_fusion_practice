...
class ManualControl(Node):
    ...
    def __init__(self, resolution):
        super(ManualControl, self).__init__("manual_control")
        self._surface = None
        ...
        self.lane_invasion_subscriber = self.create_subscription(
            CarlaLaneInvasionEvent, "/carla/{}/lane_invasion".format(self.role_name),
            self.on_lane_invasion, 10)
...
class KeyboardControl(object):
    ...
    def __init__(self, role_name, hud, node):
        ...
        # ROS 2 QoS Profiles
        fast_qos = rclpy.qos.QoSProfile(depth=10)
        fast_latched_qos = rclpy.qos.QoSProfile(
            depth=10,
            durability=rclpy.qos.DurabilityPolicy.TRANSIENT_LOCAL)

        self.vehicle_control_manual_override_publisher = self.node.create_publisher(
            Bool,
        ...
        self.auto_pilot_enable_publisher = self.node.create_publisher(
            Bool,
            "/carla/{}/enable_autopilot".format(self.role_name),
            qos_profile=fast_qos)

        self.vehicle_control_publisher = self.node.create_publisher(
            CarlaEgoVehicleControl,
            "/carla/{}/vehicle_control_cmd_manual".format(self.role_name),
            qos_profile=fast_qos)

        self.carla_status_subscriber = self.node.create_subscription(
            CarlaStatus,
            "/carla/status",
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == K_q:
                    ...
                    self._control.gear = 1 if not self._control.reverse else -1
                elif event.key == K_m:
    ...
    def _on_new_carla_frame(self, data):
        ...
        if not self._autopilot_enabled and self.vehicle_control_manual_override:
            try:
                ...
                self.node.get_logger().warn("Could not send vehicle control: {}".format(error))
...
class HUD(object):
    ...
    def __init__(self, role_name, width, height, node):
        ...
        self.vehicle_status_subscriber = node.create_subscription(
            ...
            self.vehicle_status_updated, 10)

        self.vehicle_info = CarlaEgoVehicleInfo()
        
        # ROS 2 Transient Local QoS Profile for info
        info_qos = rclpy.qos.QoSProfile(
            depth=10,
            durability=rclpy.qos.DurabilityPolicy.TRANSIENT_LOCAL)

        self.vehicle_info_subscriber = node.create_subscription(
            CarlaEgoVehicleInfo,
        ...
        self.gnss_subscriber = node.create_subscription(
            NavSatFix,
            "/carla/{}/gnss".format(self.role_name),
            self.gnss_updated,
            10)

        self.odometry_subscriber = node.create_subscription(
            Odometry,
    ...
    def update_info_text(self):
        ...
        # ROS 2 clock representation
        time_seconds = float(self.node.get_clock().now().nanoseconds / 1e9)
        time_str = str(datetime.timedelta(seconds=time_seconds))[:10]

        if self.carla_status.fixed_delta_seconds:
            fps = 1 / self.carla_status.fixed_delta_seconds
        self._info_text = [
            'Frame: % 22s' % self.carla_status.frame,
            'Simulation time: % 12s' % time_str,
            'FPS: % 24.1f' % fps, '',
